...
\item The total_score attribute is NA for all rows where the rank is in a range. Therefore, the total_score attribute is ignored. The world_rank attribute will be used in its place, as it essentially represents the same thing.
\end{itemize}


\subsubsection{CWUR Data}
The CWUR data is by far the cleanest dataset being used in this report. There are 200 missing values for \textit{broad_impact}, which are imputed using the per-country mean for that attribute.
