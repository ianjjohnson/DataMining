...
The summary for the test transaction set is ommited in the interest of brevity.
...
The same can be said for the yield range. The most frequent yield range in the check data is the highest possible yield range. This indicates that high-yield seeds are more likely to go to market, as the seeds at markey being used for testing have high yield. Therefore, at this point it can be, on a working basis, inferred that high-yield and low-rm seeds are optimal for going to market.
...
In this set of itemsets, we see less meaningful data than in the prior, as each itemset includes a locationGroup and year item, which are soemwhat novel attributes that, in isolation, don't provide much insight to our analysis.
...
@
\textit{Figure 2.5 - Top 5 most frequent itemsets, exluding "." items, for the test data}
\end{center}

Figure 2.5 shows the top 5 itemsets by frequency from the test dataset, exluding missing (".") items. Much like for the test dataset, we can see that these itemsets are not particularly meaningful, as they are all single-item itemsets representing years and location groups.
...
<<matcheck, echo=FALSE, fig=TRUE, include=FALSE, width=4, height=4>>=
plot(rules.check, method="grouped", main="Check Set")
@
<<mattest,  echo=FALSE, fig=TRUE, include=FALSE, width=4, height=4>>=
plot(rules.test, method="grouped", main="Test Set")
@

\begin{center}
\includegraphics[width=7cm, height=7cm]{project3-matcheck}
\includegraphics[width=7cm, height=7cm]{project3-mattest}
...
<<graphcheck,  echo=FALSE, fig=TRUE, include=FALSE, width=4, height=4>>=
plot(head(rules.check, n=20), method="graph", control=list(type="items"),main="Check Set")
@

<<graphtest,  echo=FALSE, fig=TRUE, include=FALSE, width=4, height=4>>=
plot(head(rules.test, n=20), method="graph", control=list(type="items"), main="Test Set")
@

\begin{center}
\includegraphics[width=7cm, height=7cm]{project3-graphcheck}
\includegraphics[width=7cm, height=7cm]{project3-graphtest}
